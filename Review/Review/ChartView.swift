...
// body안에는 뷰 선언 코드만!, 함수는 body 밖에 선언
    ...
    VStack {
      ...
      LazyVGrid(columns: Array(repeating: GridItem(.flexible()), count: 7)) {
       // ForEach(arrayMonthDays(for: currentMonth), id: \.self) { date in
        ForEach(0..<arrayMonthDays(for: currentMonth).count, id: \.self) { index in
...
#Preview {
  ChartView(markedDates: Set([
    Calendar.current.startOfDay(for: Date()),
    Calendar.current.startOfDay(for: Calendar.current.date(byAdding: .day, value: -3, to: Date())!),
    Calendar.current.startOfDay(for: Calendar.current.date(byAdding: .day, value: -6, to: Date())!),
    Calendar.current.startOfDay(for: Calendar.current.date(byAdding: .day, value: -13, to: Date())!),
    Calendar.current.startOfDay(for: Calendar.current.date(byAdding: .day, value: -15, to: Date())!)
  ]))
}
